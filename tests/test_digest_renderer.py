...
class StructuredDigestRendererTest(unittest.TestCase):
    ...
    def test_builds_delivery_body_and_graph_send_payload(self) -> None:
        ...
        self.assertIn('name="color-scheme" content="light"', payload.delivery_payload["html_body"])
        self.assertIn("background:#ffffff!important", payload.delivery_payload["html_body"])
        self.assertIn("border-left:4px solid #dc2626", payload.delivery_payload["html_body"])
        self.assertIn("background:#fffafa!important", payload.delivery_payload["html_body"])
