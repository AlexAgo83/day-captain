...
- Keep the routine weekday cron path on `trigger-hosted-job --job morning-digest` with a default target time of `08:45 Europe/Paris`, keep the Sunday recap path on `trigger-hosted-job --job weekly-digest`, and reserve `validate-hosted-service` for manual checks and rollout validation.
