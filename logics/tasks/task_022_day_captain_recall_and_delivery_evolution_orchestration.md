...
- Real hosted proof is now complete on Sunday, March 8, 2026 against `https://day-captain.onrender.com`:
  - `GET /healthz` reported `graph_auth_mode=app_only`, `storage_backend=postgres`, `configured_sender_user=daycaptain@circle-mobility.com`, and `email_command_allowed_senders=[alexandre.agostini@circle-mobility.com]`
  - `validate-hosted-service --target-user alexandre.agostini@circle-mobility.com --check-email-command --email-command-sender alexandre.agostini@circle-mobility.com --email-command-text recall-week` completed successfully
  ...
  - hosted `email-command-recall` returned run `2d4c9a596e834f7c8dba84ff02340874` with `command_name=recall-week`, `target_user_id=alexandre.agostini@circle-mobility.com`, and `delivery_mode=graph_send`
