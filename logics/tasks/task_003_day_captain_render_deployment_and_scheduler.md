...
- Validation results:
  ...
  - live Render proof -> `GET /healthz` returned `200`, protected runtime validation reported `graph_auth_mode=app_only` and `storage_backend=postgres`, and `validate-hosted-service` completed successfully for `target.user@company.com`
- Validation results:
