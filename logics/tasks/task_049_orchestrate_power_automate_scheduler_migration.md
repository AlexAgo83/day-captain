...
- [x] 6. Validate docs and Logics corpus, record run evidence, and close the migration task only after fallback instructions are clear.
...
- [x] Power Automate scheduler runbook is present and linked from operator docs.
- [x] App and private ops docs name Power Automate as primary and GitHub workflow_dispatch as fallback.
- [x] Live Power Automate evidence is recorded for morning-digest and weekly-digest.
- [x] Active GitHub Actions schedule triggers are disabled only after evidence exists.
...
# Validation
- Passed: `logics-manager flow validate req_054_day_captain_power_automate_scheduler_migration --fixable --explain`.
- Passed: `logics-manager lint --require-status`.
- Passed: `logics-manager audit --group-by-doc`.
- Passed: live morning and weekly Power Automate runs completed before GitHub schedule triggers were removed; `workflow_dispatch` fallback remains available.
- Power Automate morning and weekly flows succeeded; GitHub schedules were removed while workflow_dispatch fallback was preserved; flow validation, lint, and audit passed.
